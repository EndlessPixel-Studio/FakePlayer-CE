...
public class EnderchestCommand extends AbstractCommand {
    ...
    @Inject
    public EnderchestCommand(InvseeManager invseeManager) {
     ...
     * 打开假人的末影箱
     */
    public void enderchest(@NotNull Player sender, @NotNull CommandArguments args) throws WrapperCommandSyntaxException {
        ...
        if (!Objects.equals(sender.getLocation().getWorld(), fake.getLocation().getWorld())) {
            throw CommandAPI.failWithString(ComponentUtils.toString(
                    translatable("fakeplayer.command.enderchest.error.not-the-same-world"),
                    TranslatorUtils.getLocale(sender)
            ));
        }
        invseeManager.enderchest(sender, fake);
    }
