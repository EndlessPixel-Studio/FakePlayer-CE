...
- Added an independent `replace_tools` feature that swaps a worn main-hand tool for the highest-durability item of the same type in the inventory, regardless of enchantments. The old tool is kept in the replacement's slot; the remaining-durability threshold defaults to 10.
...
- 新增独立的 `replace_tools` 特性：主手工具耐久较低时，从背包换入同类型且耐久最高的工具，附魔差异不影响匹配；旧工具放入替换品原槽位，剩余耐久阈值默认是 10。
