...
- **Build: added PMD and SpotBugs static analysis.** `pmdMain` and `spotbugsMain` now run on `fakeplayer-api` and `fakeplayer-core`. The PMD ruleset is a curated high-signal whitelist (currently `UseLocaleWithCaseConversions`, which caught and fixed locale-less case conversions in command tab-completion). Both tasks only report for now (`ignoreFailures = true`) so existing code is not blocked while findings are triaged.
...
- **构建：接入 PMD 与 SpotBugs 静态分析。** 在 `fakeplayer-api` 与 `fakeplayer-core` 上新增 `pmdMain` 与 `spotbugsMain` 任务。PMD 规则集采用高信噪比白名单（目前含 `UseLocaleWithCaseConversions`，已借此修复指令补全里缺失 Locale 的大小写转换）。两项任务目前仅生成报告、不阻断构建（`ignoreFailures = true`），便于团队在排查存量告警的同时不阻塞原有代码。
