...
- Fixed command tab-completion in non-English locales (e.g. Turkish `tr_TR`): case conversions now use `Locale.ROOT`, so suggestions are correct regardless of the server locale.
...
- 修复非英文区域（如土耳其语 `tr_TR`）下的指令补全：大小写转换现统一使用 `Locale.ROOT`，无论服务端区域设置如何，补全建议都正确。
