...
- **新增指令 `/fp enderchest`**（别名 `/fp ec`）：打开假人的末影箱，判定规则与 `/fp invsee` 完全一致 —— 查看者需与假人处于同一世界；OP 或假人的创建者即使没有权限也允许打开（除非关闭 `allow-non-op-open-inv`）。新增权限节点 `fakeplayer.command.enderchest`，已纳入 `fakeplayer.spawn` 权限组。
